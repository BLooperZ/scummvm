...
bool EditableWidget::handleKeyDown(Common::KeyState state) {
	...
	case Common::KEYCODE_BACKSPACE:
		deleteIndex = Common::convertBiDiU32String(_editString + " ").getLogicalPosition(_caretPos);
		if (deleteIndex > 0) {
			deleteIndex--;
			_editString.deleteChar(deleteIndex);
			setCaretPos(Common::convertBiDiU32String(_editString + " ").getVisualPosition(deleteIndex));
			dirty = true;
	...
	case Common::KEYCODE_END:
		// Move caret to end
		setCaretPos(Common::convertBiDiU32String(_editString + " ").getVisualPosition(_editString.size()));
		forcecaret = true;
		dirty = true;
		break;
	...
	case Common::KEYCODE_HOME:
		// Move caret to start
		setCaretPos(Common::convertBiDiU32String(_editString + " ").getVisualPosition(0));
		forcecaret = true;
		dirty = true;
		break;
		...
		if (state.flags & Common::KBD_CTRL) {
			if (g_system->hasTextInClipboard()) {
				...
				for (uint32 i = 0; i < text.size(); ++i) {
					int logicalPosition = Common::convertBiDiU32String(_editString + " ").getLogicalPosition(_caretPos);
					if (tryInsertChar(text[i], logicalPosition)) {
						setCaretPos(Common::convertBiDiU32String(_editString + " ").getVisualPosition(logicalPosition + 1));
					}
				}
				dirty = true;
			}
